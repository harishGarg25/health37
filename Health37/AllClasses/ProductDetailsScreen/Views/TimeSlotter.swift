...
class TimeSlotter {
  ...
  func setPossibleAppointments() {
    ...
    let possibleAppointments = stride(from: 0, to: openMinutes, by: appointmentInterval).compactMap { Calendar.current.date(byAdding: .minute, value: $0, to: openTime) }
  ...
  func createDate(withDay day: Int, hour: Int, minute: Int) -> Date {
    return DateComponents(calendar: Calendar.current, year: Calendar.current.component(.year, from: Date()), month: Calendar.current.component(.month, from: Date()), day: day, hour: hour, minute: minute, second: 0).date!
  }
  
  func createCompleteDate(year: Int, month: Int, withDay day: Int, hour: Int, minute: Int) -> Date {
    return DateComponents(calendar: Calendar.current, timeZone: TimeZone.current, year: year, month: month, day: day, hour: hour, minute: minute, second: 0).date!
  }
}
